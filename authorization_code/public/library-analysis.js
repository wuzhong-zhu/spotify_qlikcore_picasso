...
export default function (access_token,GUID){
      $.ajax({
        ...
        for (var i=0;i<trackData.items.length;i++) {
        	tempAlbums.push(trackData.items[i].track.album.id);
          	tempArtists.push(trackData.items[i].track.artists[0].id);
        }
        ...
        promises.push($.ajax({
	        ...
	        headers: { 'Authorization': 'Bearer ' + access_token},
	        data:{ids: tempAlbums.slice(0,20).toString()}
		}));
        promises.push($.ajax({
	        url: 'https://api.spotify.com/v1/artists',
	        headers: { 'Authorization': 'Bearer ' + access_token},
	        data:{ids: tempArtists.slice(0,20).toString()}
		}));
        promises.push($.ajax({
	        url: 'https://api.spotify.com/v1/albums',
	        headers: { 'Authorization': 'Bearer ' + access_token},
	        data:{ids: tempAlbums.slice(20,40).toString()}
		}));
        promises.push($.ajax({
	        url: 'https://api.spotify.com/v1/artists',
	        headers: { 'Authorization': 'Bearer ' + access_token},
	        data:{ids: tempArtists.slice(20,40).toString()}
		}));
        ...
        Promise.all(promises).then(function(values) {
        	//Consolidate data
		  	...
		  	tempAlbums=tempAlbums.concat(values[2].albums);
		  	tempArtists=tempArtists.concat(values[3].artists);
        	...
        	var artistGenrePairs=[];
		 	for (var i=0;i<trackData.items.length;i++) {
				var tempAddDate = trackData.items[i].added_at.substring(0,10);
				var tempReleaseYear = tempAlbums[i].release_date.substring(0,4);
		        tracks.push(new track(trackData.items[i].track.album.name,
		        						trackData.items[i].track.album.name,
		        						trackData.items[i].track.artists[0].name,
		        						trackData.items[i].track.duration_ms,
		        						trackData.items[i].track.name,
		        						trackData.items[i].track.popularity,
		        						i+1));
		        albums.push(new album(tempAlbums[i].name,
	          	...
	          	var tempImg= null;
	          	if (tempArtists[i].images.length>0){
	          		tempImg=tempArtists[i].images[0].url;
	          	}
	        ...
	        enigma.create(config).open().then((qix) => {
	          qix.createSessionAppUsingHalyard(halyard).then((result) =>{
	            ...
	            var old_element = document.getElementById("btn-clear");
	            ...
	            result.getAppLayout()
	              ...
	              // Graph 1 ended
	              // Graph 5
	              result.createSessionObject(genreProperty).then((model) => {
	              	const object5=model;
                    const update5 = () => object5.getLayout().then((layout) => {
                		...
                		var settings = ({
						    scales: {
						      x: {
						        data: { field: '=Count(name)' }
						      },
						      y: {
						        data: { field: 'genre' }
						      },
						      col: {
						        data: { extract: { field: 'genre' } },
						        type: 'color'
						      }
						    },
						...
						settings.interactions = [
						  {
						    ...
						    events: {
						      ...
						      mousedown: function(e) {
						        this.chart.component('select').emit('select', app4,e);
						    	}
						    }
						  }
						]
                    ...
                    object5.on('changed', update5);
                    $( window ).resize(update5);
                    update5();
                  });
	              // Graph 5 end
	              // Graph 2
...
function track(addDate,albumName, artistName, duration, name, popularity,rank) {
